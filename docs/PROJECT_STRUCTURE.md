...
```
.ralph/
├── ralph.toml                 # Configuration file
├── PRD.md                     # Task tracker (Markdown)
├── AGENTS.md                  # Repo-specific commands
├── progress.md                # Append-only progress log
├── FEEDBACK.md                # Operator feedback and review notes
├── PROMPT_build.md            # Build iteration prompt
├── PROMPT_plan.md             # Planning prompt
├── PROMPT_judge.md            # Judge gate prompt
├── PROMPT_review.md           # Review gate prompt
├── specs/                     # Requirements specifications
│   └── *.md                   # Individual spec files
├── templates/                 # Custom task templates
│   └── *.json                 # Template definitions
├── permissions.json           # Optional authorization rules
├── state.json                 # Session state (machine-readable)
├── logs/                      # Per-iteration agent logs
│   └── iteration-*.log        # Timestamped log files
├── receipts/                  # Execution receipts
│   ├── command-*.json         # Command receipts
│   └── evidence-*.json        # Evidence citations
├── context/                   # Context snapshots
│   └── <iteration>/           # Per-iteration snapshots
│       └── ANCHOR.md          # Anchor context file
├── attempts/                  # Task attempt records
│   └── <task-id>/             # Per-task attempt directories
│       └── attempt-*.json     # Attempt records
├── snapshots/                 # Git-based snapshots
...
```
.ralph/
├── ralph.toml          ✅ Created (config)
├── PRD.md              ✅ Created (empty template)
├── AGENTS.md           ✅ Created (template)
├── progress.md         ✅ Created (empty)
├── FEEDBACK.md         ✅ Created (empty)
├── PROMPT_*.md         ✅ Created (templates)
├── specs/              ✅ Created (empty dir)
├── templates/          ✅ Created (empty dir)
└── state.json          ❌ Not yet (created on first run)
```
...
├── receipts/           ✅ Created (dir)
│   ├── command-*.json  ✅ Created
│   └── evidence-*.json ✅ Created (if evidence found)
├── context/            ✅ Created (dir)
│   └── 1/ANCHOR.md     ✅ Created
└── attempts/           ✅ Created (dir)
    └── <task-id>/      ✅ Created
        └── attempt-*.json ✅ Created
```
...
```
.ralph/
├── PRD.md              🔄 Updated (tasks marked complete)
├── progress.md         🔄 Appended (progress logs)
├── state.json          🔄 Overwritten (each iteration)
├── logs/               📊 Grows (N log files)
├── receipts/           📊 Grows (2N receipt files)
├── context/            📊 Grows (N directories)
└── attempts/           📊 Grows (varies by retries)
```
