...
def cmd_interventions(args: argparse.Namespace) -> int:
    ...
    if args.json:
        ...
        if args.latest:
            ...
            if rec:
                print_output(json_module.dumps(rec.to_dict(), indent=2), level="quiet")
            else:
                print_output("{}", level="quiet")
        else:
            ...
            print_output(json_module.dumps(output, indent=2), level="quiet")
        return 0
    ...
    print_output(f"  Lookback: {cfg.interventions.lookback_iterations} iterations", level="normal")
    print_output(f"  Confidence threshold: {cfg.interventions.confidence_threshold}", level="normal")
    print_output("", level="normal")
    ...
    if recs:
        ...
        print_output("", level="normal")
        print_output(
            f"Use --latest to see full details, or --json for machine-readable output.",
            level="normal"
        )
    else:
        print_output(
