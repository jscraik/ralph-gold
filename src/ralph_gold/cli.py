...
def cmd_run(args: argparse.Namespace) -> int:
    ...
    results = run_loop(root, agent=agent, max_iterations=args.max_iterations, cfg=cfg)
    for r in results:
        print(
...
def _plan_prompt(prd_filename: str, desc: str) -> str:
    ...
    lines: list[str] = []
    lines.append("You are generating a prioritized task file (plan/backlog) for the Golden Ralph Loop.")
    lines.append("")
    lines.append(f"Write or update the task file at: {prd_filename} (repo root).")
    lines.append("Do NOT implement any code in this step. Only produce the plan/task list.")
    lines.append("")
    ...
    if ext in {".md", ".markdown"}:
        ...
        lines.append("- Optional (recommended): add indented acceptance bullets under each task.")
    ...
    lines.append("")
    lines.append("After writing the task file, briefly summarize the task list you created.")
    return "\n".join(lines) + "\n"
...
def _regen_plan_prompt(project_root: Path, prd_filename: str, specs_report: str) -> str:
    ...
    lines.append("2) Perform a gap analysis: what is missing, partial, or inconsistent?")
    ...
    lines.append("- Each checkbox task must be shippable in ONE iteration / ONE commit.")
    lines.append("- Under each checkbox, add 2-5 indented bullet acceptance criteria.")
    lines.append("- Tasks must be objectively verifiable (tests/lint/typecheck/CLI commands).")
...
def cmd_plan(args: argparse.Namespace) -> int:
    ...
    start = time.time()
    timed_out = False
    timeout_s = int(cfg.loop.runner_timeout_seconds) if int(cfg.loop.runner_timeout_seconds) > 0 else None
    try:
        cp = subprocess.run(
            argv,
            cwd=str(root),
        ...
        timed_out = True
        cp = subprocess.CompletedProcess(argv, returncode=124, stdout=e.stdout or "", stderr=e.stderr or "")
...
def build_parser() -> argparse.ArgumentParser:
    ...
    p_step.add_argument("--prompt-file", default=None, help="Override files.prompt for this run (e.g. PROMPT_plan.md)")
    p_step.add_argument("--prd-file", default=None, help="Override files.prd for this run (e.g. IMPLEMENTATION_PLAN.md)")
    ...
    p_run.add_argument("--prompt-file", default=None, help="Override files.prompt for this run")
    p_run.add_argument("--prd-file", default=None, help="Override files.prd for this run")
    ...
    p_specs_check.add_argument("--specs-dir", default=None, help="Specs directory (default: from config)")
    p_specs_check.add_argument("--strict", action="store_true", help="Treat warnings as errors")
    ...
    p_plan.add_argument("--prd-file", default=None, help="Target file to write (defaults to files.prd from ralph.toml)")
    p_plan.set_defaults(func=cmd_plan)

    p_regen = sub.add_parser("regen-plan", help="Regenerate IMPLEMENTATION_PLAN.md from specs/* + codebase gap analysis")
    p_regen.add_argument("--agent", default="claude", help="Runner to use (default: claude)")
    p_regen.add_argument("--prd-file", default=None, help="Target plan file (defaults to files.prd)")
    p_regen.add_argument("--specs-dir", default=None, help="Specs directory (default: from config)")
    p_regen.add_argument("--no-specs-check", action="store_true", help="Don't run specs check before generating prompt")
